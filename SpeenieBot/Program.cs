...
namespace SpeenieBot
{
    ...
    class Program
    {
        ...
        public static double deflat = 32.8203525;
        public static double deflon = -97.011733;
        ...
        {
            //   this.Icon = new Icon(@"../SpeenieBot.Comm/config.txt");
            string cfgtxt = System.IO.File.ReadAllText(@"../Comm/config.txt");
        ...
        static private async Task bw_DoWorkTask(TelegramBotClient Bot, Message message)
        {
            ...
            if ((message.Type == Telegram.Bot.Types.Enums.MessageType.Location) && (banned.ContainsKey(message.Chat.Id.ToString()) == false))
            {
                ...
                try
                {
                    ...
                    //logging
                    string buf = "";
                    buf += @"http://wikimapia.org/#lang=en&lat=";
            ...
            else if ((message.Type == Telegram.Bot.Types.Enums.MessageType.Text) && (banned.ContainsKey(message.Chat.Id.ToString()) == false))
            {
                ...
                else if ((message.Text == "/getattractor"))
                {
                    ...
                    try
                    {
                        ...
                        try { await Bot.SendTextMessageAsync(message.Chat.Id, "Wait a minute. It may take a bot. :-)"); } catch (Exception ex) { }
               ...
               else if ((message.Text == "/getrepeller") || (message.Text == "/getvoid"))
                {
                    ...
                    try
                    {
                        ...
                        try { await Bot.SendTextMessageAsync(message.Chat.Id, "Gimme a sec..."); } catch (Exception ex) { }
                        double appi = upresets[(int)usessions[message.Chat.Id], 4];
                        double tmplat = upresets[(int)usessions[message.Chat.Id], 1];
                        double tmplon = upresets[(int)usessions[message.Chat.Id], 2];
                        int tmprad = (int)upresets[(int)usessions[message.Chat.Id], 0];
...
+ " radius: " + incoords[3].ToString("#0.00", System.Globalization.CultureInfo.InvariantCulture) + " meters";
                ...
                else if ((message.Text == "/getpair"))
                {
                    ...
                    try
                    {
                        ...
                        string mesg1 = "something's wrong";
...
+ " radius: " + incoords[6].ToString("#0.00", System.Globalization.CultureInfo.InvariantCulture) + " meters";
                ...
                else if (message.Text == "/test")
                {
                    ...
                    try { await Bot.SendTextMessageAsync(message.Chat.Id, "Speenie Bot is online"); } catch (Exception ex) { }

                }

                //start new and revised commands

                else if ((message.Text.Contains("intent ") == true)) //if ((message.Text.Contains("intent"))) 
                {

                    string intent = message.Text;
                    string toBeSearched = "intent ";
                    int ix = intent.IndexOf(toBeSearched);
                    bool issent = false;
                    try
                    {
                        if (usessions.ContainsKey(message.Chat.Id) == false)
                        {
                            ...
                            int rtr = 0;
                            while ((issent == false) && (rtr < 5) && (ix != -1))
                            {
                                string intention = intent.Substring(ix + toBeSearched.Length);
                                try
                                {

                                    await Bot.SendTextMessageAsync(message.Chat.Id, "Intent: " + intention.ToString() + Environment.NewLine);
                                    issent = true;

                                }
                                catch (Exception e) { issent = false; rtr++; Console.WriteLine("Intent is empty. Try again " + Environment.NewLine + e.Message.ToString()); }
                            }
                        }
                        ...
                        else
                        {
                            ...
                            //  while ((issent == false) && (ix != -1)) //does work, but testing error catching.
                            while (issent == false)
                            {
                                try
                                {
                                ...
                                catch (Exception e) { issent = false; Console.WriteLine("Intent is empty. Try again " + Environment.NewLine + e.Message.ToString()); }
                            }
                        ...
                        System.IO.File.AppendAllText(logpath, message.Date.ToString() + " " + message.From.Id.ToString() + " " + buf1
                               + "Intent: " + intent.ToString() + Environment.NewLine);
                    }
                    //logging

                    catch (Exception e) { Console.WriteLine("Intent setting error " + Environment.NewLine + e.Message.ToString()); }
                }
                else if ((message.Text == "/start") || (message.Text == "/help"))
                {
                    try
                    {
                        await Bot.SendTextMessageAsync(message.Chat.Id, "Speenie Bot is Started.");
                    ...
                    catch (Exception e) { Console.WriteLine("Help file error " + Environment.NewLine + e.Message.ToString()); }
                }
                else if ((message.Text == "/getcommands"))
                {
                    try
                    {
                        //  await Bot.SendTextMessageAsync(message.Chat.Id, "Speenie Bot is Started.");
                        string alltxt = System.IO.File.ReadAllText("scommands.txt", System.Text.Encoding.GetEncoding(1251));
                        await Bot.SendTextMessageAsync(message.Chat.Id, alltxt);
                    }

                    catch (Exception e) { Console.WriteLine("Something ain't right. Commands file error." + Environment.NewLine + e.Message.ToString()); }
                }
                else if ((message.Text == "/fatumtheory"))
                {
                    try
                    {
                        string theoryone = System.IO.File.ReadAllText("fatum_theory1.txt", System.Text.Encoding.GetEncoding(1251));
                     //   string theorytwo = System.IO.File.ReadAllText("fatum_theory2.txt", System.Text.Encoding.GetEncoding(1251));

                        await Bot.SendTextMessageAsync(message.Chat.Id, theoryone);
                     //   await Bot.SendTextMessageAsync(message.Chat.Id, theorytwo);
                    }

                    catch (Exception e) { Console.WriteLine("Something ain't right. Theory files error." + Environment.NewLine + e.Message.ToString()); } // 11/19/20 - Was working. Now, keep getting Telegram message length limit error
                }
                else if ((message.Text == "/showmo"))
                {
                    try
                    {
                        string alltxt = System.IO.File.ReadAllText("mobile.txt", System.Text.Encoding.GetEncoding(1251));
                        await Bot.SendTextMessageAsync(message.Chat.Id, alltxt);
                    }
                    catch (Exception e) { Console.WriteLine("Help file error " + Environment.NewLine + e.Message.ToString()); }
                }
                else if ((message.Text == "/showdesk"))
                {
                    try
                    {
                        string alltxt = System.IO.File.ReadAllText("desktop.txt", System.Text.Encoding.GetEncoding(1251));
                        await Bot.SendTextMessageAsync(message.Chat.Id, alltxt);
                    }
                    catch (Exception e) { Console.WriteLine("Help file error " + Environment.NewLine + e.Message.ToString()); }
                }
                //end of new and revised commands
                ...
                else if (message.Text.Contains(@"google.com/maps/"))
                {
                    try
                    {
                        ...
                        while ((issent == false) && (rtr < 5))
                        {
                            try
                            {
                                await Bot.SendTextMessageAsync(message.Chat.Id, "New Google location confirmed.");
                                issent = true;
                            }
                            catch (Exception e) { issent = false; rtr++; }
                        }
            ...
            while (true)
            {
                ...
                catch (Exception e1)
                {
                    ...
                    try
                    {
                        ...
                        {
                            conerrcou = 0;
                            if (IsProcessOpen("BotMan") == false)
                            {
                                if (System.IO.File.Exists("BotMan.exe"))
                                {
                                    System.Diagnostics.Process.Start("BotMan.exe");
                                }
                                System.Threading.Thread.Sleep(2000);
                            }
                            if (IsProcessOpen("BotMan") == true)
                            {
                                Environment.Exit(0);
                            }
                        }
        ...
        static void Main()
        {
            ...
            catch (Exception ex)
            {
                Console.WriteLine("There was an exception on program start: " + Environment.NewLine + ex.Message.ToString()
                  + Environment.NewLine + Environment.NewLine + "Check config.txt file settings");
